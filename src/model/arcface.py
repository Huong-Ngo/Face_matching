...
class IResNet(nn.Module):
    ...
    def forward(self, x):
        with torch.cuda.amp.autocast(self.fp16):
            ...
            x = torch.flatten(x, 1)
            x = self.dropout(x)
        x = self.fc(x.float() if self.fp16 else x).view(-1,512)
        x = self.features(x)
        return x

class Iresnet_finetune(nn.Module):
   def __init__(self, arcface_model):
    super(Iresnet_finetune, self).__init__()
...
def get_model(model_type):
    if model_type == 'iresnet50':
        return iresnet50()
    if model_type == 'iresnet100':
        return iresnet100()
...
## ft_mdl = ft_model('100', weight_path = ...)

def ft_model(model_type = '100', weight_path = '/content/drive/MyDrive/Colab_Notebooks/Model_weight/backbone_r101.pth'):
    arcface_model = get_model(model_type)
    arcface_model.load_state_dict(torch.load(weight_path, lambda s, l : s))
    return Iresnet_finetune(arcface_model)
...
class KSubArcFace(nn.Module):
    ...
    def forward(self, input: torch.Tensor, label: torch.Tensor|int):
        ...
        cosine = torch.max(cosine, dim= 2).values
...
class ArcFace_org(nn.Module):
    ...
    def __init__(self, in_features, out_features, s=30.0, m=0.50, easy_margin=False):
        super(ArcFace_org, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.s = s
        self.m = m
