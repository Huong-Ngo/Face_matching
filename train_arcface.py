...
def train_arcface(model, arcface, train_path, test_path, epochs, batch_size = 32, device = None, checkpoint_path = "weight_arc_triplet"):
    writer = SummaryWriter(f'runs/arcface_s={str(arcface.s)}_m={str(arcface.m)}_{date_time}')
    if device is None:
    ...
    def train_one_epoch(model, arcface, device, train_loader, optimizer, criterion, epoch):
        ...
        for batch_idx, (anchor, positive, negative, a_label, p_label, n_label) in enumerate(pbar := tqdm(train_loader)):
            ...
            # feature_anchor, feature_positive, feature_negative = feature_anchor.detach(), feature_positive.detach(), feature_negative.detach()
            ...
            pr_anchor = torch.argmax(out_anchor,dim = 1)
            pr_positive = torch.argmax(out_positive,dim = 1)
            pr_negative = torch.argmax(out_negative,dim = 1)
            prs_anchor = a_label == pr_anchor
            prs_positive = p_label == pr_positive
            prs_negative = n_label == pr_negative
            cat_apn = torch.cat((prs_anchor,prs_positive,prs_negative),0)
            result.append(cat_apn)
            ...
            # for name, param in model.named_parameters():
            #     param_update[name] = param
            # for name, param in model.named_parameters():
            #     result = torch.all(old_param.get(name).eq(param_update.get(name)))
            #     # print(name, result)
            ...
            if iters % log_interval == 0:
                ...
                acc_train = torch.mean(torch.cat(result,0).float()).item()
                ...
                writer.add_scalar('training_accuracy', acc_train, iters)
